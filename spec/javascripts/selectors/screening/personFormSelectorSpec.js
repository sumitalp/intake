...
describe('personFormSelectors', () => {
    ...
    it('returns formatted people object map', () => {
      ...
      const peopleForm = {
        ...
        two: {
          ...
          phone_numbers: [
            {id: null, number: {value: null}, type: {value: null}},
          ],
          addresses: [
            {
              touched: {},
      ...
      expect(getPeopleWithEditsSelector(state)).toEqualImmutable(
        fromJS({
          one: {
            id: 'one',
            ...
            races: [{race: 'Abandoned', race_detail: null}],
          },
          two: {
            id: 'two',
            legacy_descriptor: 'a legacy descriptor',
  ...
  describe('getFilteredPersonRolesSelector', () => {
    ...
    describe('when a reporter role is arleady selected', () => {
      ...
      it('returns all roles with reporter roles disabled', () => {
        expect(
          getFilteredPersonRolesSelector(state, personId)
        ).toEqualImmutable(
          fromJS([
            ...
            {label: 'Perpetrator', value: 'Perpetrator', disabled: false},
            {
              ...
              value: 'Mandated Reporter',
              disabled: true,
            },
            {
              label: 'Non-mandated Reporter',
              value: 'Non-mandated Reporter',
              disabled: true,
            },
            {
  ...
  describe('getErrorsSelector', () => {
    ...
    describe('roles in getErrorsSelector', () => {
      it('returns roles error if role includes Victim and date of birth or approximate age is empty', () => {
        ...
        const state = fromJS({
          screeningInformationForm: {
            started_at: {value: '2018-01- v:24:22.007Z'},
          },
          peopleForm,
        })
        expect(
      ...
      it('does not return roles error if role includes Victim and date of birth is under 18', () => {
        const peopleForm = {
          1: {
            date_of_birth: {value: '12/24/2001'},
            roles: {value: ['Victim'], errors: []},
          },
        }
        const state = fromJS({
          screeningInformationForm: {
      ...
      it('returns roles error if role includes Victim and date of birth is over 18', () => {
        const peopleForm = {
          1: {
            date_of_birth: {value: '12/24/1996'},
            roles: {value: ['Victim'], errors: []},
          },
        }
        const state = fromJS({
      ...
      it('does not return roles error if role includes Victim and approximate age with units is under 18', () => {
        const peopleForm = {
          1: {
            approximate_age: {value: '17'},
            approximate_age_units: {value: 'years'},
            roles: {value: ['Victim'], errors: []},
          },
          2: {
            approximate_age: {value: '214'},
            approximate_age_units: {value: 'months'},
            roles: {value: ['Victim'], errors: []},
          },
          3: {
            approximate_age: {value: '923'},
            approximate_age_units: {value: 'weeks'},
            roles: {value: ['Victim'], errors: []},
          },
          4: {
            approximate_age: {value: '6522'},
            approximate_age_units: {value: 'days'},
            roles: {value: ['Victim'], errors: []},
          },
        }
      ...
      it('returns roles error if role includes Victim and approximate age with units is under 18', () => {
        const peopleForm = {
          1: {
            approximate_age: {value: '18'},
            approximate_age_units: {value: 'years'},
            roles: {value: ['Victim'], errors: []},
          },
          2: {
            approximate_age: {value: '217'},
            approximate_age_units: {value: 'months'},
            roles: {value: ['Victim'], errors: []},
          },
          3: {
            approximate_age: {value: '940'},
            approximate_age_units: {value: 'weeks'},
            roles: {value: ['Victim'], errors: []},
          },
          4: {
            approximate_age: {value: '6575'},
            approximate_age_units: {value: 'days'},
            roles: {value: ['Victim'], errors: []},
          },
        }
  ...
  describe('getRolesSelector', () => {
    it('returns roles with error if it has one', () => {
      ...
      const state = fromJS({peopleForm})
      expect(getRolesSelector(state, '1')).toEqualImmutable(
        fromJS({
          value: ['Victim'],
          errors: [],
        })
  ...
  describe('getLastNameSelector', () => {
    it('returns last name', () => {
      const peopleForm = {
        ...
        2: {
          roles: {value: ['Other role', 'Some role']},
          last_name: {value: 'Smith2', errors: []},
        },
      }
      const state = fromJS({peopleForm})
      expect(getLastNameSelector(state, '1')).toEqual(
        fromJS({
          value: 'Smith',
          errors: [],
          required: true,
      ...
      expect(getLastNameSelector(state, '2')).toEqual(
        fromJS({
          value: 'Smith2',
          errors: [],
          required: true,
  ...
  describe('getFirstNameSelector', () => {
    it('returns first name', () => {
      const peopleForm = {
        ...
        2: {
          roles: {value: ['Other role', 'Some role']},
          first_name: {value: 'Jessy', errors: []},
        },
      }
      const state = fromJS({peopleForm})
      expect(getFirstNameSelector(state, '1')).toEqual(
        fromJS({
          value: 'John',
          errors: [],
          required: true,
      ...
      expect(getFirstNameSelector(state, '2')).toEqual(
        fromJS({
          value: 'Jessy',
          errors: [],
          required: true,
  ...
  describe('getSocialSecurityNumberSelector', () => {
    it('returns the social security number and any relevent errors', () => {
      ...
      const state = fromJS({peopleForm})
      expect(getSocialSecurityNumberSelector(state, '1')).toEqual(
        fromJS({
          value: '123-45-6789',
          errors: [],
        })
  ...
  describe('getTouchedFieldsForPersonSelector', () => {
    it('returns the contactForm field names that are touched', () => {
      ...
      const state = fromJS({peopleForm})
      expect(getTouchedFieldsForPersonSelector(state, '1')).toEqualImmutable(
        Seq(['fieldB', 'fieldD'])
      )
    })
  ...
  describe('getVisibleErrorsSelector', () => {
    it('returns an error if the first name has a validation and is touched', () => {
      ...
      const state = fromJS({peopleForm})
      expect(
        getVisibleErrorsSelector(state, '1').get('first_name')
    ...
    it('returns an error if the last name has a validation and is touched', () => {
      ...
      const state = fromJS({peopleForm})
      expect(
        getVisibleErrorsSelector(state, '1').get('last_name')
    ...
    it('does not return an error if the last name has not been touched', () => {
      ...
      const state = fromJS({peopleForm})
      expect(getVisibleErrorsSelector(state).get('last_name')).toEqualImmutable(
        List()
      )
    })
    it('does not return an error if the first name has not been touched', () => {
      ...
      const state = fromJS({peopleForm})
      expect(
        getVisibleErrorsSelector(state).get('first_name')
  ...
  describe('getPersonWithEditsSelector', () => {
    ...
    it('returns formatted person object map', () => {
      const personId = 'one'
      expect(getPersonWithEditsSelector(state, personId)).toEqualImmutable(
        fromJS({
          id: 'one',
          legacy_descriptor: 'a legacy descriptor',
          screening_id: '123456',
    ...
    describe('ssb', () => {
      it('adds ssb info to participant children', () => {
        expect(
          getPersonWithEditsSelector(state, 'two').get(
