...
describe('personShowSelectors', () => {
  ...
  describe('getFormattedPersonInformationSelector', () => {
    it('returns a blank person when person does not exist', () => {
      ...
      expect(
        getFormattedPersonInformationSelector(state, '2')
      ).toEqualImmutable(
        fromJS({
    ...
    it('includes the legacy source for the given person', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('legacySource')
    ...
    it('includes the csec details for the given person', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('csecStartedAt')
      ).toEqual(
        fromJS({
          value: '11/11/1111',
          errors: [],
    ...
    it('does not include the csec details if not provided for the given person', () => {
      const participants = [{id: '1', csec: []}]
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('csecEndedAt')
      ).toEqual(
        fromJS({
          value: undefined,
          errors: [],
    ...
    it('includes the display name for the given person', () => {
      const participants = [
        {id: '1', first_name: 'John', middle_name: 'Q', last_name: 'Public'},
      ]
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('name')
      ).toEqual(
        fromJS({
          value: 'John Q Public',
          errors: [],
    ...
    it('includes the gender for the given person', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').getIn([
          'gender',
          'value',
        ])
      ).toEqual('Intersex')
    ...
    it('includes the formatted languages for the given person', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('languages')
    ...
    it('does not include approximate age if person has a date of birth', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('approximateAge')
      ).toEqual(undefined)
    })

    it('includes the approximate age for the given person', () => {
      const participants = [
        {id: '1', approximate_age: '9', approximate_age_units: 'dog years'},
      ]
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('approximateAge')
    ...
    it('includes the value and empty errors for the social security number for the given person', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('ssn')
      ).toEqualImmutable(
        fromJS({
          value: '987-65-4321',
          errors: [],
    ...
    it('includes the formatted races for the given person', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('races')
      ).toEqual(
        'White - Romanian (primary), Asian - Chinese, Black or African American'
      )
    })

    it('shows primary race when there is one race', () => {
      const participants = [
        {id: '1', races: [{race: 'White', race_detail: 'Romanian'}]},
      ]
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('races')
      ...
      const participants = [
        {
          id: '1',
          ethnicity: [
            {hispanic_latino_origin: 'Yes', ethnicity_detail: ['Mexican']},
          ],
        },
      ]
      const state = fromJS({participants})
      expect(
        getFormattedPersonInformationSelector(state, '1').get('ethnicity')
      ).toEqual('Mexican - Yes')
    })
      ...
      const participants = [
        {
          id: '1',
          ethnicity: [
            {hispanic_latino_origin: 'Unknown', ethnicity_detail: []},
          ],
        },
      ]
      const state = fromJS({participants})
  ...
  describe('getFormattedPersonWithErrorsSelector', () => {
    it('includes errors for ssn', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonWithErrorsSelector(state, '1').get('ssn')
      ).toEqualImmutable(
        fromJS({
          value: '987-65-4321',
    ...
    it('includes errors and required value for name', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonWithErrorsSelector(state, '1').get('name')
      ).toEqualImmutable(
        fromJS({
    ...
    it('includes errors for gender', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonWithErrorsSelector(state, '1').get('gender')
      ).toEqualImmutable(
        fromJS({
          value: undefined,
    ...
    it('includes errors for dateOfBirth', () => {
      ...
      const state = fromJS({participants})
      expect(
        getFormattedPersonWithErrorsSelector(state, '1').getIn([
          'dateOfBirth',
          'errors',
        ])
  ...
  describe('getPersonFormattedPhoneNumbersSelector', () => {
    it('returns info for the person with the passed id', () => {
      ...
      expect(
        getPersonFormattedPhoneNumbersSelector(state, '1')
          .first()
          .get('type')
      ).toEqual('Home')
    })
    ...
    it('returns the extension for a number', () => {
      const people = [
        {id: '1', phone_numbers: [{type: 'Home', extension: '1234'}]},
      ]
      const state = fromJS({participants: people})
      expect(
        getPersonFormattedPhoneNumbersSelector(state, '1')
      ...
      const people = [
        {
          id: '1',
          addresses: [
            {type: '32', legacy_descriptor: {legacy_id: '23'}},
            {type: 'Cell'},
          ],
        },
        {
          id: '2',
          addresses: [{type: 'Cell', legacy_descriptor: {legacy_id: '33'}}],
        },
      ]
      const state = fromJS({participants: people})
      expect(
        getReadOnlyPersonFormattedAddressesSelector(state, '1').size
      ).toEqual(1)
    })
  })
  ...
  describe('getPersonAlertErrorMessageSelector', () => {
    it('returns alert if roles include Victim and firstName is empty', () => {
      const people = [
        {id: '1', roles: ['Victim', 'other role'], last_name: 'Smith'},
      ]
      const state = fromJS({participants: people})
    ...
    it('returns alert if roles include Victim and lastName is empty', () => {
      const people = [
        {id: '1', roles: ['Victim', 'other role'], first_name: 'John'},
      ]
      const state = fromJS({participants: people})
      expect(getPersonAlertErrorMessageSelector(state, '1')).toEqual(
        'Alleged victims must be identified with a name, even Doe or Unknown, and must be under the age of 18'
  ...
  describe('getErrorsSelector', () => {
    describe('zip', () => {
      it('must be 5 digits long', () => {
        ...
        const state = fromJS({participants: people})
        expect(getErrorsSelector(state, 'one').get('addressZip')).toBeImmutable(
          List([['zip code should be 5 digits']])
        )
      })
      it('should return empty when zip is 5 digits', () => {
        ...
        const state = fromJS({participants: people})
        expect(
          getErrorsSelector(state, 'one').get('addressZip')
        ).toEqualImmutable(List([]))
      })
    ...
    describe('social security number', () => {
      it('must be 9 digits long', () => {
        ...
        const state = fromJS({participants: people})
        expect(getErrorsSelector(state, 'one').get('ssn')).toEqualImmutable(
          List(['Social security number must be 9 digits long.'])
        )
      })
      ...
      it('cannot begin with 9.', () => {
        ...
        const state = fromJS({participants: people})
        expect(getErrorsSelector(state, 'one').get('ssn')).toEqualImmutable(
          List(['Social security number cannot begin with 9.'])
        )
      })
      ...
      it('cannot begin with 666.', () => {
        ...
        const state = fromJS({participants: people})
        expect(getErrorsSelector(state, 'one').get('ssn')).toEqualImmutable(
          List(['Social security number cannot begin with 666.'])
        )
      })
      ...
      it('cannot contain all 0s in the first group', () => {
        ...
        const state = fromJS({participants: people})
        expect(getErrorsSelector(state, 'one').get('ssn')).toEqualImmutable(
          List(['Social security number cannot contain all 0s in a group.'])
        )
      })
    ...
    describe('gender', () => {
      it('is required if role is Victim', () => {
        ...
        const state = fromJS({participants: people})
        expect(getErrorsSelector(state, 'one').get('gender').size).toEqual(1)
      })
      ...
      it('has no error if it is provided', () => {
        ...
        const state = fromJS({participants: people})
        expect(getErrorsSelector(state, 'one').get('gender')).toEqualImmutable(
          List()
        )
      })
    })
    ...
    describe('dateOfBirth', () => {
      it('returns no error if date is current or in the past', () => {
        ...
        const state = fromJS({participants: people})
        expect(
          getErrorsSelector(state, 'one').get('dateOfBirth')
        ).toEqualImmutable(List())
      })
    ...
    describe('csec types', () => {
      it('returns no error if there is a csec type', () => {
        ...
        const state = fromJS({
          screeningInformationForm: {report_type: {value: 'csec'}},
          participants: [participant],
          peopleForm: {one: person},
        })
        expect(
          getErrorsSelector(state, 'one').get('csecTypes')
        ).toEqualImmutable(List([]))
      })
    ...
    describe('csec started at', () => {
      it('returns no error if there is a csec start date', () => {
        ...
        expect(
          getErrorsSelector(state, 'one').get('csecStartedAt')
        ).toEqualImmutable(List([]))
      })
      ...
      it('returns error csec start date is in the future', () => {
        ...
        expect(
          ...
          List(['The start date and time cannot be in the future.'])
        )
      })
      it('returns error csec start date is after end date', () => {
        ...
        const participant = {
          id: 'one',
          csec: [{start_date: futureDate, end_date: todayDate}],
        }
        const person = {id: 'one', roles: {value: ['Victim']}}
        const state = fromJS({
          screeningInformationForm: {report_type: {value: 'csec'}},
    ...
    describe('csec ended at', () => {
      it('returns no error if there no csec start date', () => {
        ...
        expect(
          getErrorsSelector(state, 'one').get('csecEndedAt')
        ).toEqualImmutable(List([]))
      })
      ...
      it('end date cannot be in future', () => {
        ...
        expect(
          ...
          List(['The end date and time cannot be in the future.'])
        )
      })
    })
    ...
    describe('roles in getErrorsSelector', () => {
      it('returns roles error if role includes Victim and date of birth or approximate age is empty', () => {
        ...
        const state = fromJS({
          screeningInformationForm: {
            started_at: {value: '2018-01-31T00:24:22.007Z'},
          },
          participants: people,
        })
        expect(
          getErrorsSelector(state, '1')
            .get('roles')
            .first()
        ).toEqual('Alleged victims must be under 18 years old.')
      })

      it('does not return roles error if role includes Victim and date of birth is under 18', () => {
        const people = [
          {id: '1', date_of_birth: '12/24/2001', roles: ['Victim']},
        ]
        const state = fromJS({
          screeningInformationForm: {
            started_at: {value: '2018-01-31T00:24:22.007Z'},
        ...
        expect(getErrorsSelector(state, '1').get('roles')).toEqualImmutable(
          List()
        )
      })
      ...
      it('does not return roles error if role includes Victim and date of birth is in the future', () => {
        const people = [
          {id: '1', date_of_birth: '2/24/2018', roles: ['Victim']},
        ]
        const state = fromJS({
          screeningInformationForm: {
            started_at: {value: '2018-01-31T00:24:22.007Z'},
      ...
      it('returns roles error if role includes Victim and date of birth is over 18', () => {
        const people = [
          {id: '1', date_of_birth: '12/24/1998', roles: ['Victim']},
        ]
        const state = fromJS({
          screeningInformationForm: {
            started_at: {value: '2018-01-31T00:24:22.007Z'},
        ...
        expect(getErrorsSelector(state, '1').get('roles')).toEqualImmutable(
          List(['Alleged victims must be under 18 years old.'])
        )
      })
      ...
      it('does not return roles error if role includes Victim and approximate age with units is under 18', () => {
        ...
        expect(getErrorsSelector(state, '2').get('roles')).toEqualImmutable(
          List([])
        )
        expect(getErrorsSelector(state, '3').get('roles')).toEqualImmutable(
          List([])
        )
        expect(getErrorsSelector(state, '4').get('roles')).toEqualImmutable(
          List([])
        )
      })
    ...
    it('returns last name error if last name is empty and role includes Victim', () => {
      ...
      expect(
        getErrorsSelector(state, '1')
          .get('name')
          .first()
      ).toEqual('Please enter a last name.')
    })
    ...
    it('returns first name error if first name is empty and role includes Victim', () => {
      ...
      expect(
        getErrorsSelector(state, '1')
          .get('name')
          .first()
      ).toEqual('Please enter a first name.')
    })

    it('returns last name error if last name is empty and role includes Perpetrator', () => {
      const people = [
        {id: '1', roles: ['Perpetrator', 'other role'], first_name: 'John'},
      ]
      const state = fromJS({participants: people})
      expect(
        getErrorsSelector(state, '1')
    ...
    it('returns first name error if first name is empty and role includes Perpetrator', () => {
      const people = [
        {id: '1', roles: ['Perpetrator', 'other role'], last_name: 'Smith'},
      ]
      const state = fromJS({participants: people})
      expect(
        getErrorsSelector(state, '1')
    ...
    it('returns first name error and last name error if first name and last name are empty and role includes Victim', () => {
      ...
      expect(
        getErrorsSelector(state, '1')
          .get('name')
          .includes('Please enter a first name.')
      ).toEqual(true)
      expect(
        getErrorsSelector(state, '1')
          .get('name')
          .includes('Please enter a last name.')
      ).toEqual(true)
    })
  })
})
