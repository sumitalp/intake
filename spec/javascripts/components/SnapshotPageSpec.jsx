...
describe('SnapshotPage', () => {
  const renderSnapshotPage = ({participants = [], ...args}) => {
    ...
    return shallow(<SnapshotPage {...props} />, {
  ...
  it('renders history of involvement', () => {
    const snapshotPage = renderSnapshotPage({})
    expect(snapshotPage.find('Connect(HistoryOfInvolvement)').exists()).toBe(
      true
    )
  })
  ...
  it('renders person search', () => {
    ...
    expect(
      snapshotPage.find('Connect(PersonSearchForm)').props().isClientOnly
    ).toBe(true)
  })

  it('renders a person card for each participant', () => {
    const snapshotPage = renderSnapshotPage({
      participants: [{id: '3'}, {id: '5'}],
    })
  ...
  it('passes the page title to the header', () => {
    ...
    expect(snapshotPage.find('Connect(PageHeader)').props().pageTitle).toEqual(
      'Snapshot'
    )
  })
  ...
  it('passes a null button to the page header so it does not render the default button', () => {
    const snapshotPage = renderSnapshotPage({})
    expect(
      snapshotPage.find('Connect(PageHeader)').props().button.type
    ).toEqual('button')
  })
