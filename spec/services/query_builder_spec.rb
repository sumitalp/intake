...
describe QueryBuilder do
  ...
  let(:person_search_fields_with_full_name_dob_address) do
    { last_name: 'last name',
      first_name: 'first name',
      middle_name: 'middle name',
      suffix: 'suffix',
  ...
  let(:person_search_fields_with_last_name_dob_address) do
    { last_name: 'last name',
      suffix: 'suffix',
      date_of_birth: '1989/05/05',
      city: 'city_search_term',
  ...
  let(:person_search_fields_with_first_name_dob_address) do
    { first_name: 'first name',
      suffix: 'suffix',
      date_of_birth: '1989/05/05',
      city: 'city_search_term',
  ...
  let(:full_name_date_of_birth_address_query) do
    PersonSearchResultBuilder.new.full_name_date_of_birth_address_query
  end
  let(:last_name_date_of_birth_address_query) do
    PersonSearchResultBuilder.new.last_name_date_of_birth_address_query
  end
  let(:first_name_date_of_birth_address_query) do
    PersonSearchResultBuilder.new.first_name_date_of_birth_address_query
  end
  ...
  describe '#build' do
    ...
    context 'when advanced search feature flag is on' do
      ...
      it 'returns query with full name, ssn, date of birth, and address' do
        ...
        expect(result['_source']).to eq full_name_date_of_birth_address_query['_source']
        expect(result['size']).to eq full_name_date_of_birth_address_query['size']
        expect(result['sort']).to eq full_name_date_of_birth_address_query['sort']
        expect(
          result['track_scores']
        ).to eq full_name_date_of_birth_address_query['track_scores']
        expect(result['query']).to eq full_name_date_of_birth_address_query['query']
      end

      it 'returns query with last name, ssn, date of birth, and address' do
        result = described_class.build(
          is_advanced_search_on: 'true',
          person_search_fields: person_search_fields_with_last_name_dob_address
        ).payload.as_json
        expect(result['_source']).to eq last_name_date_of_birth_address_query['_source']
        expect(result['size']).to eq last_name_date_of_birth_address_query['size']
        expect(result['sort']).to eq last_name_date_of_birth_address_query['sort']
        expect(
          result['track_scores']
        ).to eq last_name_date_of_birth_address_query['track_scores']
        expect(result['query']).to eq last_name_date_of_birth_address_query['query']
      end

      it 'returns query with first name, ssn, date of birth, and address' do
        result = described_class.build(
          is_advanced_search_on: 'true',
          person_search_fields: person_search_fields_with_first_name_dob_address
        ).payload.as_json
        expect(result['_source']).to eq first_name_date_of_birth_address_query['_source']
        expect(result['size']).to eq first_name_date_of_birth_address_query['size']
        expect(result['sort']).to eq first_name_date_of_birth_address_query['sort']
        expect(
          result['track_scores']
        ).to eq first_name_date_of_birth_address_query['track_scores']
        expect(result['query']).to eq first_name_date_of_birth_address_query['query']
      end
    end
